...
sortedByBirthYear = inventors.sort((first, second) => first.year - second.year);
...
firstLast = people.map((person) => {
    const first = person.split(', ')[1];
    const second = person.split(', ')[0];
    return `${first} ${second}`
})
...
isAdultPresent = devs.some((dev) => {
  return currentYear - dev.year > 18 ? true : false
})
...
isEveryone19OrOlder = devs.every((dev) => {
    return currentYear - dev.year >= 19 ? true : false
  })
